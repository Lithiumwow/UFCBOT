"""American odds math + parlay combination."""
...
def american_to_decimal(american: int | float) -> Decimal:
    """Convert American odds to decimal (inclusive of stake)."""
...
def decimal_to_american(decimal_odds: Decimal | float) -> int:
    """Convert decimal odds back to nearest American integer."""
...
def imply_prob(american: int) -> Decimal:
    """Naive implied probability (no juice split)."""
    d = american_to_decimal(american)
    return (Decimal(1) / d).quantize(Decimal("0.0001"))


def combine_parlay(american_legs: Sequence[int]) -> dict:
    """
    Multiply independent legs into a parlay price.

    Returns dict with decimal, american, and per-leg breakout.
    """
