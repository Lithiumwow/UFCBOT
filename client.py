...
DEFAULT_UA = "UFC-Tracker/1.0 (+https://fightodds.io/odds)"
...
class FightOddsClient:
    ...
    def event_fights_by_name(self, event_name_fragment: str, first: int = 50) -> list[Fight]:
        ...
        Week numbers are exact (\"Week 1\" never matches \"Week 10\").
        """
        import re
        ...
        hits = self.search_events(frag, first=max(first, 20))
        week_m = re.search(r"\bweek\s*(\d+)\b", frag, flags=re.I)
        want_week = int(week_m.group(1)) if week_m else None

        def _ok(name: str) -> bool:
            low = name.lower()
            fl = frag.lower()
            if fl not in low and not all(
                t in low for t in re.findall(r"[a-z0-9]+", fl) if len(t) > 2
            ):
                # loose fallback: token-ish
                pass
            if want_week is not None:
                mw = re.search(r"\bweek\s*(\d+)\b", name, flags=re.I)
                if mw is None or int(mw.group(1)) != want_week:
                    return False
            return fl in low or (
                "contender" in fl
                and "contender" in low
                and want_week is not None
            )

        match = None
        # Prefer exact contender-style title first
        for e in hits:
            if want_week is not None and re.search(
                rf"\bweek\s*{want_week}\b", e.name, flags=re.I
            ):
                if "contender" in frag.lower() and "contender" not in e.name.lower():
                    continue
                match = e
                # Prefer name that includes the year digits from the query if any
                ym = re.search(r"\b(20\d{2})\b", frag)
                if ym and ym.group(1) in (e.name or ""):
                    break
                if match and ym is None:
                    break
        if match is None:
            for e in hits:
                if _ok(e.name):
        ...
        if match is None:
            for e in self.upcoming_events(first=60):
                if want_week is not None:
                    mw = re.search(r"\bweek\s*(\d+)\b", e.name, flags=re.I)
                    if mw is None or int(mw.group(1)) != want_week:
                        continue
                if frag.lower() in e.name.lower():
                    match = e
                    break
